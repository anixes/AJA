...
def test_execution_success_streams_and_persists_logs(tmp_path):
    async def scenario():
        ...
        assert (Path(result.manifest_path).parent / "stdout.log").read_text(encoding="utf-8").strip() == "hello"
...
def test_execution_nonzero_exit_records_failure(tmp_path):
    async def scenario():
        ...
        root = make_git_project(tmp_path)
        manager = ExecutionManager(project_root=root)
        result = await manager.run(ExecutionRequest(command=py_cmd("import sys; sys.exit(7)"), timeout=30))
...
def test_telemetry_sink_failure_does_not_fail_execution(tmp_path):
    async def scenario():
        ...
        result = await manager.run(ExecutionRequest(command=py_cmd("print('ok')"), timeout=30))
