...
Welcome, fellow agent! This file provides a comprehensive context summary, architectural maps, and recent changes so you can immediately begin developing, testing, or debugging the AJA & AJA system without friction.

---

## 🎯 System Overview & Scope

**AJA** is an enterprise-grade agentic orchestration engine designed to manage autonomous multi-agent missions with extreme observability, safety controls, and multi-tenant telemetry.
* **Core Language**: Python (running under **global Python 3.12.10**).
* **Runtime Core**: `libs/aja-core` containing configuration validation, safe plan simulation (`--dry-run`), systems diagnostics (`doctor`), and the baton handover subsystem.
* **Native Rust Integration**: PyO3 GIL-free IPC engine (`aja_native`) which serializes execution batons into Apache Arrow format.
...
* **Schema Definition**: [config_schema.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/config_schema.py) defining models for territories, swarms, models, settings, and agent configs.
* **Import Hook**: Modified [config.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/config.py) to validate `aja.json` against the schema. If validation fails, it outputs warning details and safely loads strict, secure default settings rather than failing silently or throwing unexpected runtime exceptions.
...
* **Trace Propagation**: Created `TraceContextManager` in [telemetry.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/observability/telemetry.py). It tracks a thread-local and asyncio-local context variable `_trace_id_ctx`.
* **Arrow IPC Integration**: The `BatonManager` in [handover.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/runtime/handover.py) automatically serializes active `trace_id` values into Arrow metadata headers upon capturing, and recovers/restores them into the current context during pickup.
...
* **Module**: [diagnostics.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/utils/diagnostics.py).
...
* **CLI Endpoint**: Run `/doctor` via `aja doctor` to verify environment readiness (configuration schema, native PyO3 engine, LanceDB tables, API tokens, and minimum memory/disk space).

### 4. Interactive Scaffolding & Setup Wizard
* **Onboarding Tool**: Built `aja setup` inside [main.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/main.py), utilizing rich-prompts to scaffold the standard workspace layout.
...
We recently completed the Phase 1 Architectural Upgrades, transitioning AJA into a pure, local-first terminal CLI/TUI assistant:

### 1. Pluggable Model and Tool Interfaces (ABCs)
* **Decoupled Extensions**: Created `BaseModelProvider` and `BaseTool` abstract base classes in [interfaces.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/api/interfaces.py).
...
* **Session Broker**: Built [gateway_runner.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/gateway/gateway_runner.py) to manage concurrent multi-platform communication adapters.
...
* **LanceDB Cron**: Built [cron_scheduler.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/scheduler/cron_scheduler.py) to parse standard 5-field cron strings (`* * * * *`) and recurring intervals (`every 30s`), preserving job state inside LanceDB databases.
...
* **Bulletproof Viewport Layout**: Replaced the cluttered browser GUI with a premium local Curses terminal TUI dashboard in [curses_tui.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/tui/curses_tui.py) containing a live HTN Plan tree DAG, tailing logs view, and metrics panel.
...
* **Network-Scalable IPC**: Extended [handover.py](file:///d:/AgenticAI/Project1(no-name)/libs/aja-core/aja/runtime/handover.py) with base64 Apache Arrow serializers.
...
* **Polite & Proactive Conversational Flow**: Upgraded LLM prompts in `libs/aja-core/aja/api/bridge.py` and `libs/aja-core/aja/gateway/orchestrator.py` to follow a premium hacker-butler secretary persona. AJA communicates with elite developer fluency, polite but proactive phrasing, and witty, concise updates.
...
* **Safe Intent Parsing Integration**: Refactored the core intent parser prompt in `libs/aja-core/aja/interface/intent_parser.py` to match the butler persona while strictly preserving the JSON output schema (`type`, `goal`, `command`, `response`, `confidence`) required for code mapping.
...
* **State Awareness**: The interactive `aja` CLI chat loop now maintains a rolling 15-turn conversation history fed directly to the intent parser, enabling fully contextual multi-step dialogues and resolving memory resets.
...
```powershell
$env:PYTHONPATH="libs/aja-core"; & "C:\Users\Asus\AppData\Local\Programs\Python\Python312\python.exe" -m aja tui --dry-run
```
...
* **Git Status Integrity**: Keep code clean. Avoid leaving behind unhandled temporary baton files. Temporary simulation batons should be written in `libs/aja-core/temp_batons` or the local system tmp directory.
