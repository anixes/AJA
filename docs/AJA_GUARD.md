...
3.  **Operator-in-the-Loop**: `Ask` commands are paused as structured approval objects. The user can approve or reject from CLI, dashboard, or Telegram.
...
5.  **AI Risk Analysis**: The Python AJA Guard path can still call the **Unified Gateway** (NVIDIA, Groq, etc.) to generate plain-language explanations for risky commands before approval.
