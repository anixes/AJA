...
## Overview
Phase 6 transforms Assistant from a task tracker into an **Executive Assistant** with judgment-based prioritization and rigorous execution constraints. 

Assistant now refuses to delegate work without a clear "Definition of Done," ensuring that delegated workers (Swarm Agents, Agent Workers, etc.) have precise success criteria.

## 1. Executive Priority Engine
Assistant now utilizes a multi-factor decision layer to decide what actually matters, rather than just listing tasks by date.
...
- **Delegatability**: Automated judgment on whether a task should be handled by the User (Operator), Assistant (Secretary), or a Worker (Swarm/Agent Worker).
...
- **Urgency Challenge**: Assistant explicitly asks if the task can be safely deferred (e.g., "Can this wait until Friday?").
- **Delegation Recommendation**: Advice on who should execute.

## 2. Definition of Done (DoD) Framework
Assistant enforces a "Quality-First" delegation pattern. Every mission launched from the dashboard or phone must include a checklist of success criteria.

### Auto-Generation Heuristics
If no DoD is provided, Assistant's backend (`api_bridge.py`) utilizes a keyword matcher to generate relevant criteria:
...
2. **Pending Approvals**: Assistant Guard security gate and Communication drafts.
