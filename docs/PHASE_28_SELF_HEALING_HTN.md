...
### Implementation Details (`agentx.planning.models`)
...
### Implementation Details (`agentx.planning.planner`)
...
- **DAG Validation Check**: Each candidate plan is run through the `DAGValidator` (`agentx.planning.dag_validator.py`), which uses strict Boolean and structural matching.
