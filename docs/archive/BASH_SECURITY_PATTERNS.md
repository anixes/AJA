...
Agent uses a multi-layered approach to ensure that shell commands are safe before execution. This document deconstructs the patterns found in `agent_guard.py` and the core security layers.
...
- Agent's `stripAllLeadingEnvVars()` logic removes `PORT=3000`.
...
- `cd ../../etc && cat shadow`
- Agent normalizes the `cd` target. If the resulting path is a protected system directory, it blocks the command chain entirely.
...
To maintain security in Agent, we follow this sequence:
