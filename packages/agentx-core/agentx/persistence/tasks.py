...
def fetch_pending_tasks(limit: int = 10) -> List[Dict]:
    ...
    results = table.search().where("status IN ('INTERRUPTED', 'PENDING', 'FAILED')").limit(limit).to_list()
...
def is_logical_task_completed(logical_task_id: str) -> bool:
    # In Pure AgentX, we search metadata for logical links
    table = _manager.get_table("core_tasks")
    results = table.search().where(f"status = 'COMPLETED' AND task_id LIKE '%{logical_task_id}%'").limit(1).to_list()
    return len(results) > 0

def cleanup_old_tasks(ttl_days: int = 30):
    # Columnar cleanup is typically done via fragment pruning
    pass
