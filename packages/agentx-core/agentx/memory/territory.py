import os
import json
import logging
from pathlib import Path
from typing import List, Dict, Any
from agentx.config import PROJECT_ROOT
from agentx.memory.secretary import get_aja_memory

logger = logging.getLogger(__name__)

class TerritoryScanner:
    """
    Scans defined territories and indexes them into AJA Memory for RAG.
    """

    SUPPORTED_EXTENSIONS = {".py", ".ts", ".js", ".json", ".md", ".txt", ".sql", ".yaml", ".yml"}
    EXCLUDE_DIRS = {"node_modules", ".git", "__pycache__", "dist", "build", ".next", ".agentx"}

    def __init__(self, territories: List[Dict[str, Any]] = None):
        if territories is None:
            # Load from agentx.json
            config_path = PROJECT_ROOT / "agentx.json"
            if config_path.exists():
                with open(config_path, "r") as f:
                    cfg = json.load(f)
                    self.territories = cfg.get("territories", [])
            else:
                self.territories = []
        else:
            self.territories = territories

    async def scan_all(self):
        """Walks through all territories and indexes files."""
        logger.info(f"AJA Memory: Starting territory scan for {len(self.territories)} locations.")
        mem = get_aja_memory()
        
        for territory in self.territories:
            rel_path = territory.get("path")
            if not rel_path:
                continue
            
            abs_path = PROJECT_ROOT / rel_path
            if not abs_path.exists():
                logger.warning(f"Territory path does not exist: {abs_path}")
                continue
            
            await self._scan_directory(abs_path)
            
        logger.info("AJA Memory: Territory scan complete.")

    async def _scan_directory(self, root: Path):
        mem = get_aja_memory()
        for path in root.rglob("*"):
            if any(part in self.EXCLUDE_DIRS for part in path.parts):
                continue
            
            if path.is_file() and path.suffix in self.SUPPORTED_EXTENSIONS:
                try:
                    content = path.read_text(encoding="utf-8", errors="ignore")
                    if not content.strip():
                        continue
                    
                    # For now, we chunk by lines or simple blocks
                    chunks = self._chunk_content(content)
                    for i, chunk in enumerate(chunks):
                        metadata = {
                            "filename": path.name,
                            "rel_path": str(path.relative_to(PROJECT_ROOT)),
                            "chunk_index": i,
                            "extension": path.suffix
                        }
                        
                        # Get embedding (placeholder/mock for now)
                        vector = self._get_embedding(chunk)
                        
                        mem.add_knowledge_chunk(
                            path=str(path.relative_to(PROJECT_ROOT)),
                            content=chunk,
                            metadata=metadata,
                            vector=vector
                        )
                except Exception as e:
                    logger.error(f"Failed to index {path}: {e}")

    def _chunk_content(self, content: str, chunk_size: int = 1000) -> List[str]:
        """Simple chunking strategy."""
        return [content[i:i+chunk_size] for i in range(0, len(content), chunk_size)]

    def _get_embedding(self, text: str) -> List[float]:
        """
        Placeholder for real embedding generation.
        In a real scenario, this would call llama-cpp or OpenAI/Gemini.
        """
        # 1536 is a common embedding size (OpenAI style)
        # We return a semi-random but deterministic vector based on text hash for testing
        import hashlib
        h = hashlib.sha256(text.encode()).digest()
        vec = [0.0] * 1536
        for i in range(min(1536, len(h))):
            vec[i] = float(h[i]) / 255.0
        return vec

if __name__ == "__main__":
    import asyncio
    logging.basicConfig(level=logging.INFO)
    scanner = TerritoryScanner()
    asyncio.run(scanner.scan_all())
