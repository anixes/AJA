...
class UnifiedGateway:
    """
    The main integration hub for the Agent.
    ...
    def __init__(self, model_id: str = "claude-3-5-sonnet"):
        ...
        self.trajectory_manager = agent_native.PyTrajectoryManager(model_id)
        self.context_threshold = 4000 # Tokens

    async def initialize(self, semantic_db_path: str = "./.agent/memory.lancedb"):
        """Initializes the native Rust semantic store."""
        try:
            agent_native.init_semantic(semantic_db_path)
            print(f"Agent: Native Semantic Memory initialized at {semantic_db_path}")
        except Exception as e:
            print(f"Agent Warning: Native memory init skipped ({e}). Using LanceDB/Arrow fallback.")
    ...
    async def chat(self, user_input: str) -> str:
        ...
        # 2. Native Context Optimization (Agent Native Optimization)
        ...
        if analysis["should_compress"]:
            print(f"Agent [Native]: Pressure detected. Optimizing trajectory via Dynamic Compression...")
        ...
        request = UniversalRequest(
            ...
            instructions=[ContentBlock(type="text", text="You are an Agent, a high-leverage autonomous orchestrator.")]
        )
        ...
        response_text = f"Agent [Rust-Core Integrated]: Reasoning about '{user_input}'"
    ...
    def compress_trajectory(self, messages: List[Dict[str, str]], start: int, end: int) -> List[Dict[str, str]]:
        ...
        summary_text = f"[AGENT COMPRESSION: {len(middle)} turns offloaded to LanceDB Semantic Store]"
    ...
    async def spawn_sub_agent(self, agent_id: str, task: str) -> str:
        ...
        print(f"Agent: Spawning sub-worker '{agent_id}' with mission baton '{code}'")
        
        # Detached background process
        subprocess.Popen(
            [sys.executable, "-m", "agent", "pickup", code],
