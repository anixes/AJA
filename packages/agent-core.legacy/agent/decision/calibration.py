...
from agent.decision.metrics import update_evaluator_performance
...
def run_calibration_tests(tracker=None) -> Dict[str, Any]:
    try:
        from agent.decision.evaluator import evaluate_combined, EVALUATORS, evaluate_semantic
    except ImportError:
    ...
    for task in tasks:
        ...
        try:
            from agent.decision.evaluator import get_evaluation_context
...
def evaluate_evaluator() -> List[str]:
    from agent.decision.metrics import get_evaluator_metrics
    issues = []
