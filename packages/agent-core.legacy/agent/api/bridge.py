...
from agent.tui.tasks import (
    STATUS_PENDING,
    STATUS_RUNNING,
    STATUS_FAILED,
    STATUS_COMPLETED,
)
from agent.memory.secretary import (
    SecretaryMemory,
...
RUNTIME_STATE_PATH = Path(".agent") / "runtime-state.json"  # debug export only
BATON_DIR = PROJECT_ROOT / "temp_batons"
API_TOKEN = os.getenv("AGENTX_API_TOKEN", "dev-token-123")
TELEGRAM_HISTORY_PATH = Path(".agent") / "telegram-history.jsonl"
TELEGRAM_PENDING_PATH = Path(".agent") / "telegram-pending.json"  # debug export only
APPROVAL_AUDIT_PATH = Path(".agent") / "approval-audit.jsonl"   # debug export only
TELEGRAM_BOT_TOKEN = os.getenv("TELEGRAM_TOKEN") or os.getenv("TELEGRAM_BOT_TOKEN", "")
TELEGRAM_ALLOWED_USER_ID = os.getenv("TELEGRAM_ALLOWED_USER_ID", "")
TELEGRAM_WEBHOOK_SECRET = os.getenv("TELEGRAM_WEBHOOK_SECRET", "")
TELEGRAM_COMMAND_TIMEOUT = int(os.getenv("TELEGRAM_COMMAND_TIMEOUT", "60"))
SECRETARY_MEMORY_DIR = Path(".agent") / "lancedb"
...
def save_telegram_pending(data: dict):
    """No-op: Telegram approvals now live in core_approvals table."""
    # Debug export only
    try:
...
# get_secretary_memory is now imported from agent.memory.secretary
...
def generate_definition_of_done(objective: str) -> list[str]:
    ...
    Assistant refuses to delegate without clear success criteria.
    """
    o = objective.lower()
...
def execute_secretary_command_sync(text: str, source: str, owner: str = "Assistant"):
    ...
    if lowered in {"task review", "review tasks", "secretary review"}:
        ...
        tasks = memory.list_tasks(statuses=[STATUS_PENDING, STATUS_RUNNING, "BLOCKED"], limit=10)
    ...
    if lowered in {
        ...
        for item in ignore:
            ...
            lines.append("")
        lines.append("Assistant will remind you if anything escalates.")
        return "\n".join(lines).strip()
...
def build_supported_command(text: str):
    ...
    if normalized in {"/start", "help", "/help"}:
        return {
            ...
            "message": "\n".join(
                [
                    "Assistant Telegram control is online.",
                    "Commands:",
                    "- status",
                    "- check gpu",
                    "- run training job",
                    ...
                    "- complete <task_id>",
                    "",
                    "You can also talk naturally. Assistant will answer as a chat assistant.",
    ...
    if normalized == "run training job":
        return {
            "kind": "execute",
            "command": f'"{sys.executable}" agent.py run --bg "run training job"',
            "requires_confirmation": True,
            "reason": "Starts a background Assistant mission powered by Agent Core.",
    ...
    if normalized == "shutdown laptop tonight":
        ...
        return {
            "kind": "execute",
            "command": f'shutdown /s /t {delay} /c "Scheduled by Assistant Telegram"',
            "requires_confirmation": True,
...
def build_dry_run_summary(action_type: str, command: str):
    ...
    if action_type == "training_job":
        return "Would delegate a background Assistant training mission through Agent Core."
...
async def run_file_guardian_check(command: str):
    def _run():
        return subprocess.run(
            [resolve_npx_executable(), "tsx", "apps/cli-ts/src/telegram_file_guardian_check.ts", ".agent/telegram-command.txt", command],
            cwd=str(PROJECT_ROOT),
            capture_output=True,
            text=True,
            timeout=30,
...
def build_assistant_chat_context(limit: int = 5):
    memory = get_secretary_memory()
    context: list[str] = []
    try:
...
def generate_assistant_chat_reply(text: str, user_id: int, chat_id: int | str):
    system_prompt = (
        "You are Assistant, the natural-language assistant/operator for Agent Core. "
        ...
        "or the dashboard so Agent can create an approval request. "
    ...
    return (
        "I am here. Natural chat is wired through the Assistant bridge, but the model returned an empty response. "
...
async def execute_telegram_command(text: str, user_id: int, chat_id: int | str):
    ...
    if lower.startswith("/handover "):
        ...
        try:
            ...
            if session_data:
                ...
                return f"🚀 Handover Successful!\nAssistant is now linked to terminal session: {session_data.get('session_id')}\nYou are now authorized to command this instance."
            else:
                return "❌ Invalid or expired OTC. Please generate a new one from your terminal using 'agent-handover'."
    ...
    if spec["kind"] == "chat":
        ...
        reply = await asyncio.to_thread(generate_assistant_chat_reply, text, user_id, chat_id)
        append_telegram_history({"user_id": user_id, "chat_id": chat_id, "command": text, "decision": "assistant_chat"})
        return reply
    ...
    if classification["decision"] == "deny":
        ...
        return f"Command denied by Assistant Safety Gate.\n{reason_text}"
    ...
    if spec.get("requires_confirmation") or classification["decision"] == "ask" or file_guardian["decision"] == "ASK":
        ...
        # --- Assistant Brain: persist approval to LanceDB (single source of truth) ---
...
def load_runtime_state():
    """Build a legacy-compatible runtime state dict from LanceDB (Assistant Brain)."""
    mem = get_secretary_memory()
...
DELEGATION_RULES = {
    ...
    "code": "Delegate to Agent Worker",
    "debug": "Delegate to Agent Worker",
    "fix bug": "Delegate to Agent Worker",
    "refactor": "Delegate to Agent Worker",
    "test": "Delegate to Agent Worker",
...
def run_priority_engine(memory: "SecretaryMemory") -> dict:
    ...
    Score all active tasks using 5 dimensions:
      ...
      5. Delegatability   — Assistant / Agent Worker / operator
...
async def recommend_workers_api(request: Request):
    ...
    Assistant recommends — operator confirms.
    """
    body = await request.json()
...
async def get_priority_engine():
    """
    Run the Assistant Priority Engine across all active tasks.
...
async def telegram_webhook(request: Request, x_telegram_bot_api_secret_token: str | None = Header(default=None)):
    ...
    if not TELEGRAM_ALLOWED_USER_ID or str(user_id) != str(TELEGRAM_ALLOWED_USER_ID):
        append_telegram_history({"user_id": user_id, "chat_id": chat_id, "command": text, "decision": "unauthorized"})
        await send_telegram_message(chat_id, "Access denied: this Telegram user is not whitelisted for Assistant.")
        return {"ok": True}
...
async def get_approval_audit_trail(approval_id: str):
    """Return the append-only audit trail for a specific approval from core_approval_audit."""
...
async def get_runtime_events_from_db(limit: int = 50):
    """Return recent runtime events from core_events (authoritative LanceDB source)."""
...
async def swarm_run(request: Request):
    ...
    try:
        proc = subprocess.Popen(
            [sys.executable, "-m", "agent.orchestration.swarm", "--mode", "baton", "--objective", objective, "--worker", worker_id],
            cwd=str(PROJECT_ROOT),
