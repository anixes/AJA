...
from agent.config import AGENTX_DIVERSITY_BETA
...
class Planner:
    ...
    gateway : optional UnifiedGateway instance.
              If None, the planner loads it lazily from agent.gateway.
    ...
    def bias_with_strategies(self, trusted: List[Dict[str, Any]], experimental: List[Dict[str, Any]] = None, is_sandbox: bool = False, risk_level: float = 0.5):
        ...
        """
        from agent.learning.exploration import exploration_controller
    ...
    def _call_llm(self, goal: str, retrieved_context: str = "", mode: str = "default", config: Optional[Dict] = None, history: Optional[List[str]] = None) -> Optional[str]:
        ...
        try:
            from agent.planning.methods import MethodLibrary
    ...
    def generate_k_plans(self, goal: str, current_state: Dict, k: int = 3, mode: str = "default") -> List[PlanGraph]:
        ...
        """
        from agent.planning.generator import generate_candidate_plans
    ...
    def _decompose_single(self, goal: str, current_state: Optional[Dict] = None, retrieval_retry: int = 0, mode: str = "default", config: Optional[Dict] = None, history: Optional[List[str]] = None) -> PlanGraph:
        ...
        from agent.planning.scorer import estimate_complexity, COMPLEXITY_LOW, COMPLEXITY_MEDIUM, score_plan
        from agent.planning.generator import generate_candidate_plans, revise_plan
        from agent.planning.verifier import verify_plan
        from agent.planning.selector import select_plan
        ...
        try:
            from agent.retrieval.retriever import retrieve
            from agent.retrieval.validator import validate_answer
        ...
        from agent.planning.simulation import select_best_simulated_plan
    ...
    def decompose(self, goal: str, current_state: Optional[Dict] = None) -> PlanGraph:
        ...
        from agent.observability.metrics import metrics_system
        ...
        if agent.config.AGENTX_DIVERSITY_BETA and should_disable_beta(metrics):
            print("[Planner] [BETA] Auto-disabling beta due to metric degradation.")
            agent.config.AGENTX_DIVERSITY_BETA = False

        if not agent.config.AGENTX_DIVERSITY_BETA:
            return self._original_decompose(goal, current_state)

        from agent.planning.scorer import estimate_complexity, COMPLEXITY_LOW
        complexity = estimate_complexity(goal)
        if complexity == COMPLEXITY_LOW:
        ...
        try:
            from agent.memory.experience_store import experience_store
        ...
        from agent.decision.disagreement import disagreement_score, detect_conflicts, minority_veto, update_disagreement_metrics
        from agent.planning.verifier import verify_plan
        from agent.planning.selector import select_plan
        from agent.decision.critic import compare_reasoning, critique_plan, critic_score
        ...
        if d_score > 0.5 or conflicts or veto or has_shared_error:
            ...
            from agent.planning.scorer import score_plan
            scored = []
        ...
        import random
        try:
            from agent.rl.policy_store import policy_store
