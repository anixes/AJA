...
from agent.runtime.sandbox import execute_command
from agent.utils.tokenjuice import TokenJuice

class AssistantGuard:
    """
    The Assistant Guard (formerly SafeShell) intercepts and inspects commands for security risks
    before Agent execution.
    """
    
    # Tier 1: Truly dangerous binaries that almost always need analysis
    DANGEROUS_BINARIES = {
        "dd", "mkfs", "shutdown", "reboot", "chmod", "chown", "kill", "pkill"
    }

    # Tier 2: Sensitive binaries that are common in dev but need monitoring
    SENSITIVE_BINARIES = {
        "rm", "mv", "wget", "curl", "bash", "sh", "zsh", "python", "npm", "gh"
    }
    ...
    def is_known_safe(self, cmd_str: str, root: str, args: list) -> bool:
        ...
        if root == "rm":
            ...
            if any(t in cmd_str for t in safe_targets) and not ("/" in cmd_str and len(cmd_str.split("/")) < 3):
                return True
        return False
    ...
    def check_and_execute(self, cmd_str: str):
        ...
        network_cmds = ["npm install", "pip install", "cargo build", "gh ", "git clone", "curl", "wget"]
        ...
        try:
            ...
            tool_context = None
            if "npm" in cmd_str: tool_context = "npm/install"
            elif "pip" in cmd_str: tool_context = "pip/install"
            ...
            if compacted_out: print(compacted_out)
            if compacted_err: print(f"Error: {compacted_err}")
...
if __name__ == "__main__":
    print("--- 🛡️ Welcome to Assistant Guard ---")
    ...
    shell = AssistantGuard(provider, key, model)
    
    while True:
        try:
            cmd = input("\nAssistant Guard > ").strip()
    ...
    print("\nAssistant Guard closed. Stay safe!")
