...
from agent.persistence.tasks import fetch_pending_tasks, enqueue_scheduled_tasks, update_task_status
from agent.presence.trigger_engine import evaluate_triggers
from agent import cmd_run
...
def _run_with_planning(objective: str, task_id: int) -> bool:
    ...
    import uuid
    from agent.planning.planner import PlanTooComplexError
    from agent.persistence.tasks import update_task_error
...
try:
    from agent.persistence.tracker import log_event
except ImportError:
    def log_event(event, payload):
        pass

try:
    from agent.presence.notifier import send_notification
except ImportError:
...
def _check_stop_flag() -> bool:
    if os.path.exists(".agent/stop_loop"):
        return True
    return False
...
def _trigger_circuit_breaker(reason: str):
    ...
    with open(".agent/stop_loop", "w") as f:
