...
from agent.tui.tasks import STATUS_PENDING, STATUS_RUNNING, STATUS_FAILED
...
def get_system_state() -> dict:
    ...
    try:
        ...
        if len(arrow) > 0:
            ...
            state["active_tasks"]  = pc.sum(pc.equal(status_col, STATUS_RUNNING)).as_py() or 0
            state["pending_tasks"] = pc.sum(pc.equal(status_col, STATUS_PENDING)).as_py() or 0
            state["failed_tasks"]  = pc.sum(pc.equal(status_col, STATUS_FAILED)).as_py() or 0
        ...
        try:
            events_table = _manager.get_table("core_events")
