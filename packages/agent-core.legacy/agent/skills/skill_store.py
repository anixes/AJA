...
class SkillStore:
    """
    High-performance Skill Store powered by LanceDB and Apache Arrow.
    ...
    def _init_tables(self):
        ...
        # 1. Core Skills Table (Arrow Schema)
        ...
        if "core_skills" not in existing_tables:
            self.db.create_table("core_skills", schema=skill_schema)
        ...
        if "core_skill_sources" not in existing_tables:
            self.db.create_table("core_skill_sources", schema=source_schema)

    def save_skill(self, data: Dict[str, Any]) -> str:
        table = self.db.open_table("core_skills")
    ...
    def update_skill(self, skill_id: str, updates: Dict[str, Any]):
        ...
        if "tool_sequence" in db_updates:
            db_updates["tool_sequence_json"] = json.dumps(db_updates.pop("tool_sequence"))
