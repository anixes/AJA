...
from agent.memory.manager import MemoryManager, get_memory_manager, list_tables_defensive
...
def mark_stale_skills(stale_after_days: int = STALE_AFTER_DAYS) -> int:
    ...
    """
    from agent.skills.skill_store import mark_skills_stale
    try:
...
def _refresh_last_used(skill_id: str) -> None:
    ...
    try:
        from agent.skills.skill_store import touch_skill
...
def _execute_step(run_id: str, step: dict, step_index: int) -> tuple:
    ...
    """
    from agent.persistence.tools import ToolGuard
...
def _update_skill_metrics(skill_id: str, success: bool) -> None:
    ...
    try:
        from agent.skills.skill_store import update_skill_metrics
...
def execute_skill(
    ...
    tracker    : agent.persistence.tracker module (optional, for structured events)
