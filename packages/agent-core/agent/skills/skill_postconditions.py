...
def _penalise_confidence(skill_id: str) -> None:
    ...
    try:
        from agent.skills.skill_store import get_skill_by_id, update_skill_metrics
...
def add_postcondition(skill_id: str, postcondition: dict) -> bool:
    ...
    if not required_fields.issubset(postcondition.keys()):
        ...
        return False
    try:
        from agent.skills.skill_store import get_skill_by_id, update_skill_postconditions
        skill = get_skill_by_id(skill_id)
        if skill is None:
