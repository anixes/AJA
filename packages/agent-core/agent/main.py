"""
Agent — Unified CLI Entry Point
=================================
The central nervous system of the Agent swarm.
...
from agent.tui.kanban import render_kanban_board
...
from agent.runtime.handover import BatonManager
from agent.interface.modern import (
    console,
    print_banner,
    print_status,
    print_doctor,
...
def cmd_run(objective: str, background: bool = False):
    ...
    with mission_spinner(objective):
        from agent.orchestration.swarm import SwarmEngine
...
def cmd_status():
    ...
    tasks = []
    try:
        from agent.persistence.tasks import fetch_pending_tasks
...
def cmd_chat():
    ...
    from agent.interface.intent_parser import parse_intent
    from agent.presence.state import get_system_state

    console.print(
        "[bold cyan]Assistant:[/] Greetings. I am your Assistant. How can I assist you today?"
    )
    console.print(
    ...
    while True:
        try:
            ...
            def get_toolbar():
                p, r = task_manager.get_counts()
                from agent.presence.state import get_system_state
                state = get_system_state()
                health = "[green]OK[/green]" if state.get("is_healthy") else "[red]ERR[/red]"
                mode = "NORMAL"
                tasks = f"Tasks: {p} pending, {r} running"
                return HTML(
                    f' <style bg="ansicyan" fg="ansiblack"> <b>AGENT</b> </style> | Mode: {mode} | {tasks} | Health: {health} '
                )
            ...
            if user_input.startswith("/"):
                ...
                if cmd == "/exit":
                    console.print(
                        "[bold cyan]Assistant:[/] Farewell. Standing by for next mission."
                    )
                    break
                elif cmd == "/clear":
                    console.clear()
                    ...
                    from agent.tui.kanban import live_kanban
            ...
            with console.status("[bold cyan]Assistant is thinking...[/]"):
                state = get_system_state()
                intent = parse_intent(user_input, [], system_state=state)

                console.print(f"[bold cyan]Assistant:[/] {intent['response']}")
            ...
            console.print(
                "\n[bold cyan]Assistant:[/] Transitioning to background. Use 'agent chat' to return."
            )
            break
        except Exception as e:
...
def cmd_doctor():
    ...
    checks = [
        ...
        (
            "Native Engine",
            (PROJECT_ROOT / "packages" / "agent-native").exists(),
            "Rust/Simd Core",
        ),
        ("Runtime Dir", (PROJECT_ROOT / ".agent").exists(), ".agent/"),
...
def show_help():
    """Displays the Agent Command Suite."""
...
[yellow]metrics[/]            → View performance
    """
    console.print(Panel(help_text, title="Agent Command Suite", border_style="cyan"))
...
def main():
    ...
    if cmd == "run":
        ...
        subprocess.run([PYTHON, "-m", "agent.interface.tui"])
