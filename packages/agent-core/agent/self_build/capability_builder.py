...
import agent.config
from agent.llm import get_gateway_for_model
from agent.runtime.event_bus import bus, EVENTS

REGISTRY_FILE = agent.config.PROJECT_ROOT / ".agent" / "agent_capabilities.json"
EXPERIENCE_FILE = agent.config.PROJECT_ROOT / ".agent" / "agent_experiences.json"
...
def propose_capability(problem: str) -> Capability:
    ...
    model_name = agent.config.AGENTX_PLANNER_MODEL
