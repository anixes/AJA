...
class MCPClientManager:
    """
    Manages persistent connections to multiple MCP servers for Agent.
    """
    def __init__(self):
    ...
    async def connect_to_server(self, name: str, command: str, args: List[str]):
        ...
        try:
            ...
            print(f"Agent: Persistent connection established to MCP server '{name}'")
            
        except Exception as e:
            await stack.aclose()
            print(f"Agent Error: Failed to connect to MCP server '{name}': {e}")
            raise
...
class MCPToolCapability:
    """
    Agent Capability to interact with MCP servers.
    """
