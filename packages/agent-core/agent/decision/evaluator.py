...
def evaluate_pipeline(task_id: int, result: str, context: Dict[str, Any], stricter: bool = False, confidence: float = 1.0) -> Dict[str, Any]:
    ...
    if det_result == "FALSE_SUCCESS":
        ...
        try:
            from agent.decision.metrics import update_evaluation_metrics
    ...
    try:
        from agent.decision.metrics import get_metrics
        _live = get_metrics()
    ...
    try:
        from agent.decision.metrics import get_contextual_reliability, detect_context_drift, update_evaluator_performance
    except Exception as e:
    ...
    active_evaluators = []
    from agent.decision.calibration import compute_confidence_threshold
