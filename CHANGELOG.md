...
### Added
- Native Agentic Engine (Manager vs Worker design) via `SwarmEngine.execute_direct` using Native Tool Calling.
- New slash commands: `/goal` (background autonomous mission execution) and `/schedule` (persistent cron job registry).

### Changed
- Deprecated legacy bash-prompting and shell string parsing for Swarm planning.
- Removed `subprocess.run(shell=True)` read-only fast paths, centralizing all execution through the agentic FSM loop.
- Deprecated `router.py` in favor of Native Tool Calling.
