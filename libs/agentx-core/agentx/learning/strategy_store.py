...
STRATEGY_FILE = agentx.config.PROJECT_ROOT / ".agentx" / "agent_strategies.json"
    ...
    def load(self):
        if STRATEGY_FILE.exists():
            try:
                ...
                logger.exception("Failed to load strategies from %s", STRATEGY_FILE)

    def save(self):
        try:
            STRATEGY_FILE.parent.mkdir(parents=True, exist_ok=True)
            with STRATEGY_FILE.open("w", encoding="utf-8") as f:
                json.dump(self.strategies, f, indent=2)
        except Exception:
            logger.exception("Failed to save strategies to %s", STRATEGY_FILE)
