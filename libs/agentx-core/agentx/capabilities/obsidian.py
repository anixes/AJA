...
class ObsidianCapability(Capability):
    ...
    def _apply_template(self, content: str, tags: List[str] = None) -> str:
        ...
        default_tags = ["agent", "long-term-memory"]
        if tags:
        ...
        header += "source: agent-orchestrator\n"
        header += "---\n\n"
    ...
    async def execute(self, action: str, **kwargs) -> CapabilityResult:
        ...
        try:
            if action == "write_note":
                ...
                return CapabilityResult(success=True, output=f"Agent: Note '{name}' archived to vault.")
                ...
                with open(path, "a", encoding="utf-8") as f:
                    ...
                    f.write(f"\n\n### [{timestamp}] Log Entry\n{content}")
                return CapabilityResult(success=True, output=f"Agent: Appended entry to '{name}'.")
    ...
    def get_description(self) -> str:
        return "Manage long-term readable knowledge in Obsidian."
