...
from aja.persistence.tasks import fetch_pending_tasks, enqueue_scheduled_tasks, update_task_status
from aja.presence.trigger_engine import evaluate_triggers
from aja import cmd_run

# ---------------------------------------------------------------------------
# Phase 11 — Planning Layer feature flag
# Set AJA_PLANNING_ENABLED=1 in the environment or agent.json to enable.
# ---------------------------------------------------------------------------
PLANNING_ENABLED: bool = os.environ.get("AJA_PLANNING_ENABLED", "0").strip() == "1"
...
def _run_with_planning(objective: str, task_id: int) -> bool:
    ...
    import uuid
    from aja.planning.planner import PlanTooComplexError
    from aja.persistence.tasks import update_task_error
...
try:
    from aja.presence.notifier import send_notification
except ImportError:
