...
class LocalExtractor:
    ...
    def extract_python(self, path):
        try:
            ...
            logger.exception("Failed to extract Python structure from %s", path)
    ...
    def extract_ts(self, path):
        try:
            ...
            logger.exception("Failed to extract TypeScript structure from %s", path)

    def _path_filter(self, path: Path) -> str:
        str_path = str(path).replace("'", "''")
        return f"path = '{str_path}'"
    ...
    def run(self):
        ...
        for root, _, files in os.walk(PROJECT_ROOT):
            if any(x in root for x in ["node_modules", ".git", "graphify-out", ".aja", "dist", "__pycache__"]):
                continue
            for f in files:
                path = Path(root) / f
                ...
                # Check cache
                cached = table.search().where(self._path_filter(path)).to_list()
                ...
                if cached:
                    table.update(where=self._path_filter(path), values=new_data)
                else:
                    table.add([new_data])
        
        logger.info("Mapped %s structural nodes across Agent codebase.", len(self.nodes))
