...
from aja.config import AJA_CRITIC_MODEL, AJA_PLANNER_MODEL, AJA_WORKER_MODEL
...
class GoalSession:
    ...
    async def _summarize_progress(self, objective: str, iteration: int) -> None:
        ...
        recent_turns = self.session.session_history[
            -6:
        ...
        async def _loop():
            for i in range(1, self.max_iterations + 1):
                ...
                if status == "complete":
                    ...
                    console.print(
                        "[dim]The agent determined this goal cannot be completed. Review manually.[/dim]"
                    )
                    return
...
class GoalSwarmSession:
    ...
    def __init__(
        self, dry_run: bool = False, max_iterations: int = 8, timeout_seconds: int = 600
    ):
        self.planner_engine = SwarmEngine(model=AJA_PLANNER_MODEL, dry_run=dry_run)
        self.critic_engine = SwarmEngine(model=AJA_CRITIC_MODEL, dry_run=dry_run)
        self.dry_run = dry_run
        self.max_iterations = max_iterations
        self.timeout_seconds = timeout_seconds
    ...
    async def run(self, objective: str) -> None:
        async def _loop():
            ...
            for i in range(1, self.max_iterations + 1):
                ...
                # Plan + Execute
                await self.planner_engine.plan_and_execute_batons(
                    full_goal, run_id=f"swarm_goal_{i}"
                )
                ...
                if status == "complete":
                    ...
                    console.print(
                        "[dim]The Swarm Critic determined this goal cannot be completed. Review manually.[/dim]"
                    )
                    return
    ...
    async def _run_critic(self, objective: str) -> str:
        ...
        history = []
        await self.critic_engine.execute_direct(
            prompt, session_history=history, interactive=False
        )
