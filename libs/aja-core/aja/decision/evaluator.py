...
EVALUATION_MODEL = os.environ.get("AJA_EVALUATION_MODEL", "secondary_model")
...
def evaluate_task(task_id: int, result: str, context: Dict[str, Any]) -> str:
    ...
    if skill and skill.get("postconditions"):
        try:
            from aja.skills.skill_postconditions import validate_postconditions
...
def meta_validate(
    ...
    try:
        ...
        meta_model = os.environ.get("AJA_META_EVALUATOR_MODEL", "claude-3-5-sonnet-20241022")
...
EVALUATORS = [
    os.environ.get("AJA_EVALUATOR_A", "gpt-4o"),
    os.environ.get("AJA_EVALUATOR_B", "claude-3-5-sonnet-20241022"),
    os.environ.get("AJA_EVALUATOR_C", "gemini-2.5-flash")
]
...
def evaluate_pipeline(task_id: int, result: str, context: Dict[str, Any], stricter: bool = False, confidence: float = 1.0) -> Dict[str, Any]:
    ...
    try:
        from aja.decision.metrics import get_metrics
        _live = get_metrics()
    ...
    try:
        from aja.decision.metrics import get_contextual_reliability, detect_context_drift, update_evaluator_performance
    except Exception as e:
    ...
    active_evaluators = []
    from aja.decision.calibration import compute_confidence_threshold
