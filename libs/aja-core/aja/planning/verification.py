...
from aja.planning.replanner import RecoveryAction
...
class SerializabilityVerifier:
    ...
    def _mock_env(self, randomize_timing: bool = False):
        ...
        return [
            patch('aja.planning.execution_bridge._cmd_run', side_effect=mocked_cmd_run),
            patch('aja.planning.plan_store.PlanStore.save'),
            patch('aja.planning.plan_store.PlanStore.record_repair'),
            patch('aja.planning.replanner.Replanner.handle_failure', side_effect=mocked_handle_failure),
        ]
