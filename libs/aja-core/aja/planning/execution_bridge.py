"""
aja/planning/execution_bridge.py
=====================================
...
No code in `aja/decision/` or `aja/persistence/` is modified.
"""
...
def _cmd_run(objective: str, context: Optional[Dict[str, Any]] = None):
    ...
    from aja.main import cmd_run  # type: ignore[import]
...
def _log_event(event: str, payload: Dict[str, Any]):
    try:
        from aja.persistence.tracker import log_event  # type: ignore[import]
...
class ExecutionBridge:
    ...
    def __init__(self, graph: PlanGraph, task_id: Optional[int] = None):
        ...
        from aja.planning.execution_log import ExecutionLog
    ...
    def _emit_debug_trace(
        ...
        if os.environ.get("AJA_DEBUG_TRACE", "0") == "1":
