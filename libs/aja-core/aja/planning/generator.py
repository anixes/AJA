...
async def generate_candidate_plans_async(goal: str, state: Dict, k: int, mode: str = "default", config: Optional[Dict] = None, history: Optional[List[str]] = None) -> List[PlanGraph]:
    """
    Async mirror of generate_candidate_plans.
    Method retrieval runs in a worker thread (embedding model load is blocking);
    LLM attempts are awaited natively; parsing/filtering stay sync (pure CPU).
    """
    import asyncio

    candidates: List[PlanGraph] = []

    # 1. Try retrieving methods first (off-loop: embedding model load)
    method_candidates = await asyncio.to_thread(retrieve_methods, goal, k)
    for m, sim in method_candidates:
        template = m.get("plan_template")
        if template and template.get("nodes"):
            try:
    ...
    # 2. If we need more candidates, generate via LLM (fresh throwaway Planner, like sync)
    from aja.planning.planner import Planner
    temp_planner = Planner()
    ...
    while len(candidates) < k and attempts < k * 3:
        ...
        try:
            raw = await temp_planner._call_llm_async(
                goal,
                retrieved_context=state.get("retrieved_context", ""),
                mode=mode,
                config=current_config,
...
async def revise_plan_async(plan: PlanGraph, feedback: Dict[str, Any], state: Optional[Dict[str, Any]] = None, max_iterations: int = 2) -> PlanGraph:
    """
    Async mirror of revise_plan.
    """
...
"""
        try:
            response = await completion_async(prompt, system_prompt="You are a strict planning refinement agent.")
            ...
            feedback = await verify_plan_async(new_plan, state=state)
