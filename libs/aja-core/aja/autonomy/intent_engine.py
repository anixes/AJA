...
SAFE_GOALS = ["retry", "optimize", "monitor", "cleanup", "notify"]

DANGEROUS_GOALS = ["delete_files", "system_modification", "external_actions"]
...
class IntentEngine:
    ...
    def generate_intents(self, state: Dict[str, Any]) -> List[Intent]:
        ...
        for goal in goal_engine.goals:
            if goal.status == "FAILED":
                intents.append(
                    Intent(
                        f"Retry failed task: {goal.objective}",
                        "retry",
                        benefit=0.8,
                        risk=0.2,
                    )
        ...
        # 2. Maintenance / Health (Only if cooldown passed)
        candidates = [
            Intent("Check system health", "monitor", benefit=0.9, risk=0.05, cost=0.1),
            Intent("Clean temp files", "cleanup", benefit=0.4, risk=0.1, cost=0.2),
            Intent(
        ...
        for i in candidates:
            last_run = self.intent_last_run.get(i.objective, 0)
    ...
    def score_intent(self, intent: Intent) -> float:
        ...
        return (
            (intent.benefit * intent.confidence) - intent.risk - intent.cost - penalty
        )
    ...
    def rank(self, intents: List[Intent]) -> List[Intent]:
        ...
        valid = [
            ...
            for s, i in scored
            if s >= self.THRESHOLD
            and i.category in SAFE_GOALS
            and not any(d in i.objective.lower() for d in DANGEROUS_GOALS)
        ]
        return sorted(valid, key=lambda i: self.score_intent(i), reverse=True)

    def safe(self, intent: Intent) -> bool:
        if intent.risk > 0.5 or intent.confidence < 0.6:
            return False
        return True
    ...
    def execute(self, intent: Intent):
        ...
        if not result["success"]:
            failure_memory.update(
                intent.objective, "root", result.get("error", "Failed"), {}, None
            )
            self.intent_failures[intent.objective] = (
                self.intent_failures.get(intent.objective, 0) + 1
            )
        ...
        if "retry" in action_desc:
            action = "retried deployment"
        elif "clean" in action_desc:
            action = "cleaned temp files"
        elif "health" in action_desc:
            action = "checked system health"
        else:
            action = action_desc
    ...
    def check_drift_control(self):
        ...
        if self.total_count >= 5:
            ...
            if success_rate < 0.8:  # Drop of 20% from ideal 1.0
                print(
                    "[IntentEngine] Success rate dropped significantly. Disabling autonomy."
                )
