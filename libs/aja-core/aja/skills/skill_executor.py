...
from aja.memory.manager import (
    MemoryManager,
    get_memory_manager,
...
def mark_stale_skills(stale_after_days: int = STALE_AFTER_DAYS) -> int:
    ...
    """
    from aja.skills.skill_store import mark_skills_stale
...
def _invoke_tool(tool_name: str, args: dict) -> tuple:
    ...
    Tool implementations live in aja/tools/<tool_name>.py and expose
    ...
    module_path = f"aja.tools.{tool_name}"
    try:
...
def _update_skill_metrics(skill_id: str, success: bool) -> None:
    ...
    try:
        from aja.skills.skill_store import update_skill_metrics
...
def execute_skill(
    ...
    tracker    : aja.persistence.tracker module (optional, for structured events)
